...
RESULTS_DIR = os.path.join(ROOT_DIR, "results/cyclone/")
...
class CycloneConfig(Config):
    ...
    NUM_CLASSES = 1 + 1  # Background + cyclone
...
class CycloneInferenceConfig(CycloneConfig):
...
class CycloneDataset(utils.Dataset):

    def load_cyclone(self, dataset_dir, subset):
        """Load a subset of the Cyclone dataset.
        dataset_dir: Root directory of the dataset.
        subset: Subset to load: train or val
        """
        # Add classes. We have only one class to add.
        self.add_class("cyclone", 1, "cyclone")
    ...
    def load_mask(self, image_id):
        ...
        mask_file = os.path.join(os.path.dirname(info['path']), info['id'].replace('.png','mask.png'))
        ...
        for num in labels:
            ...
            #if num > 1:
            #    print("image mask", num, mask_file)
        #print("size", len(mask))
        if (len(mask) == 0):
...
def train(model, dataset_dir, subset):
    """Train the model."""
    # Training dataset.
    dataset_train = CycloneDataset()
    dataset_train.load_cyclone(args.dataset, subset)
    dataset_train.prepare()

    # Validation dataset
    dataset_val = CycloneDataset()
    dataset_val.load_cyclone(args.dataset, "val")
    dataset_val.prepare()
...
def detect(model, dataset_dir, subset):
    ...
    dataset.load_cyclone(dataset_dir, subset)
    dataset.prepare()
    # Load over images
    submission = []
    for image_id in dataset.image_ids:
        ...
        gray, r['rois'], r['masks'], r['class_ids'],
            dataset.class_names, r['scores'],
            show_bbox=False, show_mask=False,
            title="Predictions", colors = ["black", "black", "black", "black", "black"])
